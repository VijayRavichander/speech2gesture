...
test_dataset = BVHFolderMotionDataset("../data/bvh2clips", window_size=480)
...
for i, (m, f) in enumerate(test_dataloader):
    ...
    test_preds.append(x_recon)
    test_indices[f].append(indices.squeeze().tolist())


df = {'filename': [], 'preds': [], 'audioname': []}
...
for k, v in test_indices.items():
    ...
    df['audioname'].append(name.replace(".bvh", ".wav"))
    df['filename'].append(name)
    df['preds'].append(v)

print(df)
df = pd.DataFrame.from_dict(df)
print("Output JSONL")
df.to_csv('./preds/test_idx.csv', index=False)
df.to_json("./preds/output.jsonl", orient="records", lines=True)
