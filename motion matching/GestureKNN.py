...
def convert_wav_to_mfcc(wav):
    mfcc_features = []
    mfcc_with_deltas = []
    
    for w, _ in wav:  # Iterate over each sequence
        w, sr = librosa.load(w, sr = None)
        ...
        hop_length = (n_samples - n_fft) // (target_frames - 1)

        mfcc = librosa.feature.mfcc(y=w, sr=16000, n_mfcc=14,  n_fft = n_fft, hop_length=hop_length)

        # Transpose to shape (n_frames, 14)
        mfcc = mfcc.T
    ...
    for mfcc in mfcc_features:
        ...
        # Concatenate MFCCs and delta features along the last axis
        combined = np.concatenate((mfcc, delta_mfcc), axis=1)
        # print(mfcc.shape, delta_mfcc.shape)
        mfcc_with_deltas.append(combined)
    
    mfcc_with_deltas = np.array(mfcc_with_deltas)
...
mfcc = convert_wav_to_mfcc(dataset)
...
# Model
gesture_knn = GestureKNN(train_features, train_motion_features, step_sz=4, random_init=True)
...
for idx in range(train_size, len(dataset)):
    ...
    new_file_name = f"./preds/{new_file_name}"
